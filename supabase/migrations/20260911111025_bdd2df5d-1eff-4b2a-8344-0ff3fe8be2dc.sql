ALTER TABLE public.page_views
  ADD COLUMN referrer_host text NOT NULL DEFAULT '',
...
CREATE INDEX page_views_referrer_host_idx ON public.page_views (referrer_host) WHERE referrer_host <> '';

CREATE OR REPLACE FUNCTION public.analytics_summary(_from date, _to date, _include_short boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
  ...
  SELECT jsonb_build_object(
    ...
    'utm_sources', COALESCE((
      ...
      FROM (
        ...
        FROM cur
        WHERE utm_source <> ''
        GROUP BY utm_source ORDER BY count(*) DESC LIMIT 12
      ) u
    ), '[]'::jsonb),
    'devices', COALESCE((
      SELECT jsonb_agg(x)
