  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Standards Document | Deerva" },
      {
        ...
        content: "A read-only document from the Deerva standards library.",
      },
...
function StandardsDocumentPage() {
  ...
  const document = useQuery({
    queryKey: ["admin", "standards", "document", slug],
    queryFn: () => getStandardsDocument({ data: { slug } }),
  });
  if (document.isLoading) return <div className="h-96 animate-pulse rounded-lg bg-muted" />;
  if (document.error || !document.data)
    return (
      <div className="space-y-4">
        <p className="text-sm text-muted-foreground">This document could not be opened.</p>
        <Button asChild variant="outline">
          <Link to="/admin/standards">Back to library</Link>
        </Button>
      </div>
    );
  ...
  return (
    <div className="w-full space-y-8">
          ...
          <Button
            ...
            onClick={() =>
              navigator.clipboard
                ...
                .then(() => toast.success("Document copied"))
            }
          >
      ...
      <article className="rounded-lg border border-border bg-card p-5 sm:p-8">
        <MessageResponse className="prose-sm max-w-none">{item.content}</MessageResponse>
      </article>
    </div>
  );
}
